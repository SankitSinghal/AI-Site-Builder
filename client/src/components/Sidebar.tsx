              ...
              if (isMessage) {
                ...
                return (
                  ...
                  isUser ? 'justify-end' : 'justify-start'}`}>
                    {!isUser && <BotIcon className="size-5 text-white" />}

                    <div
                      className={`max-w-[80%] p-2 px-4 rounded-2xl text-sm leading-relaxed ${
                        isUser
                    ...
                    {isUser && <UserIcon className="size-5 text-gray-200" />}
                  </div>
                );
              }
                ...
                <div key={ver.id}
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Link target="_blank" to={`/preview/${project.id}/${ver.id}`}>
                      <EyeIcon className="size-6 p-1 bg-gray-700 hover:bg-indigo-500 rounded transition-colors" />
                    </Link>
                  </div>
                </div>
              );
        ...
        <form onSubmit={handleRevisions} className="m-3 relative">
            <div className='flex items-center gap-2'>
                <textarea onChange={(e)=> setInput(e.target.value)} value={input} rows={4} placeholder='Describe your website or request
                 changes...' className='flex-1 p-3 rounded-xl resize-none
                 text-sm outline-none ring ring-gray-700 focus:ring-indigo-500 bg-gray-800 text-gray-100 placeholder-gray-400 
                 transition-all' disabled={isGenerating}/>
                 <button disabled={isGenerating || !input.trim()} 
                 className='absolute bottom-2.5 right-2.5 rounded-full bg-linear-to-r from-indigo-500 to-indigo-600
