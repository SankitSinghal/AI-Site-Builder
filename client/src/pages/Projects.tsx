...
import { ArrowBigDownDash, FullscreenIcon, LaptopIcon, SaveIcon, SmartphoneIcon, TabletIcon, XIcon } from 'lucide-react'
...
const Projects = () => {
  ...
  const saveProject = async () => {
    try {
      let code = previewRef.current?.getCode()
      if (!code) {
    ...
    <div className='flex flex-col h-screen w-full bg-gray-900 text-white'>
      ...
      <div className='flex max-sm:flex-col sm:items-center gap-4 px-4 py-2 no-scrollbar'>
        ...
        <div className='flex items-center justify-end gap-3 flex-1 text-xs sm:text-sm'>
          ...
          <Link target ='_blank' to={`/preview/${project.id}`} className='flex items-center gap-2 px-4 py-1 rounded sm:rounded-sm border
           border-gray-700 hover:border-gray-500 transition-colors'>
          <FullscreenIcon size={16}/>Preview
          </Link>
          <button onClick={downloadCode} className='bg-linear-to-br from-indigo-700 to indigo-600 hover:from-indigo-600 hover:to-indigo-500
           text-white px-3.5 py-1 flex items-center gap-2 rounded sm:rounded-sm transition-colors'>
            <ArrowBigDownDash size={16}/> Download</button>
