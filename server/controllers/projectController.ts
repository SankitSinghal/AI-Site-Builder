...
export const makeRevision = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const enhancedPrompt = await generateGeminiResponse(enhanceUserMessage, enhanceSystemInstruction);
    ...
    const code = await generateGeminiResponse(codeGenUserMessage, codeGenSystemInstruction);
